...
INSERT INTO tasks (title, phase, status, priority, assigned_to, due_date, start_date, notes) SELECT '1ª turma de capacitação (piloto)', 'fase_1_piloto', 'pendente', 'alta', m.id, '2026-08-20', '2026-08-01', 'Fase 1 — ago/2026. 10-15 participantes, 40 h.' FROM team_members m WHERE m.name='Thaís' AND NOT EXISTS (SELECT 1 FROM tasks WHERE title ILIKE '%1ª turma de capacitação%') LIMIT 1;
INSERT INTO tasks (title, phase, status, priority, assigned_to, due_date, start_date, notes) SELECT 'Implantar rodas-piloto no IMS', 'fase_1_piloto', 'pendente', 'alta', m.id, '2026-09-05', '2026-08-10', 'Fase 1 — ago–set/2026. Meta: 8 sessões, 40+ mulheres. QR Ouvidoria.' FROM team_members m WHERE m.name='Natalie' AND NOT EXISTS (SELECT 1 FROM tasks WHERE title ILIKE '%rodas-piloto no IMS%') LIMIT 1;
...
INSERT INTO tasks (title, phase, status, priority, assigned_to, due_date, notes) SELECT 'Termo de Cooperação MPDFT', 'fase_2_enraizamento', 'pendente', 'alta', m.id, '2026-10-08', 'Fase 2 — out/2026. Thaís articula.' FROM team_members m WHERE m.name='Thaís' AND NOT EXISTS (SELECT 1 FROM tasks WHERE title ILIKE '%Termo de Cooperação MPDFT%') LIMIT 1;
INSERT INTO tasks (title, phase, status, priority, assigned_to, due_date, notes) SELECT 'Fase final: consolidar fluxo e plano de novas rodas', 'fase_5_sustentabilidade', 'pendente', 'alta', m.id, '2027-03-20', 'Fase 5 — dez/2026–mar/2027. Fluxo, rede, novas rodas para demanda reprimida.' FROM team_members m WHERE m.name='Thaís' AND NOT EXISTS (SELECT 1 FROM tasks WHERE title ILIKE '%Fase final%consolidar%') LIMIT 1;
...
-- Fase 1 — ago–set/2026
INSERT INTO public_eventos_cal (titulo, data_evento, hora, local, tipo, descricao, confirmado) SELECT '1ª turma de capacitação (piloto) — início', '2026-08-01', 'A definir', 'Distrito Federal', 'capacitacao', '10–15 participantes, currículo de 40 horas.', false WHERE NOT EXISTS (SELECT 1 FROM public_eventos_cal WHERE titulo ILIKE '%1ª turma de capacitação%');
INSERT INTO public_eventos_cal (titulo, data_evento, hora, local, tipo, descricao, confirmado) SELECT 'Rodas-piloto — Instituto Mãos Solidárias', '2026-08-10', 'A definir', 'IMS — DF', 'roda', 'Meta: 8 sessões, 40+ mulheres. QR Code Ouvidoria MPDFT.', false WHERE NOT EXISTS (SELECT 1 FROM public_eventos_cal WHERE titulo ILIKE '%Rodas-piloto%IMS%');
